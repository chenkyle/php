...
$sql=mysql_query("select count(*) as total from tb_dingdan where xiadanren='".$_SESSION[username]."' ",$conn);
...
if($total==0){
	...
	$sql1=mysql_query("select * from tb_dingdan where xiadanren='".$_SESSION[username]."' order by time desc limit ".($page-1)*$pagesize.",$pagesize",$conn);
...
<table>
	...
	<tr>
		...
		if($page>=2)
		{
			?> <a href="member_shop_allorder.php?page=1" title="首页"><font size="3">首页</font></a>
		<a href="member_shop_allorder.php?id=<?php echo $id;?>&page=<?php echo $page-1;?>"
			title="前一页"><font size="3">前一页</font></a> <?php
		}
		if($pagecount<=4){
			for($i=1;$i<=$pagecount;$i++){
		  ?> <a href="member_shop_allorder.php?page=<?php echo $i;?>"><?php echo $i;?></a> <?php
			}
		}else{
			for($i=1;$i<=4;$i++){
		  ?> <a href="member_shop_allorder.php?page=<?php echo $i;?>"><?php echo $i;?></a> <?php }?>
		<a href="member_shop_allorder.php?page=<?php echo $page+1;?>" title="后一页"><font
			size="3">后一页</font></a> <a
			href="member_shop_allorder.php?id=<?php echo $id;?>&page=<?php echo $pagecount;?>"
