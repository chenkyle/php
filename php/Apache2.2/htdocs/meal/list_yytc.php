...
<li class="sort-menu-active"><a href="list_yytc.php?mi=1">营养套餐</a></li>
</div>

<div class="list-columns list-columns-1"> 
<!-- 商品盒子开始 -->
<?php
$sql_jr=mysql_query("select count(*) as total from tb_shangpin where tuijian=4",$conn);
	   ...
	   if($total==0){
	   ...
	   else
	    {
           ...
           $sql1=mysql_query("select * from tb_shangpin where tuijian=4 order by addtime desc limit ".($page-1)*$pagesize.",$pagesize",$conn);
...
<div class="main-bottom">
    &nbsp;&nbsp;&nbsp;&nbsp;本站共有
        ...
        <?php
		  ...
		  {
		  ?>
        <a href="list_yytc.php?tj=4&mi=1&page=1" title="首页"><font size="3">首页</font></a> <a href="list_yytc.php?tj=4&mi=1&page=<?php echo $page-1;?>" title="前一页"><font size="3">前一页</font></a>
        <?php
		  }
		   if($pagecount<=4){
		    for($i=1;$i<=$pagecount;$i++){
		  ?>
        <a href="list_yytc.php?tj=4&mi=1&page=<?php echo $i;?>"><?php echo $i;?></a>
        <?php
		     }
		   }else{
		   for($i=1;$i<=4;$i++){	 
        ...
        <a href="list_yytc.php?tj=4&mi=1&page=<?php echo $i;?>"><?php echo $i;?></a>
        <?php }?>
        <a href="list_yytc.php?tj=4&mi=1&page=<?php echo $page+1;?>" title="下一页"><font size="3">下一页</font></a> <a href="list_yytc.php?tj=4&mi=1&id=<?php echo $id;?>&page=<?php echo $pagecount;?>" title="尾页"><font size="3">尾页</font></a>
        <?php }?>
</div>
<?php }?>
<!-- 页码结束 -->
