...
$sql=mysql_query("select count(*) as total from tb_dingdan where salerid='".$salerid."'",$conn);
$info=mysql_fetch_array($sql);
$total=$info[total];
if($total==0){
	echo "本站暂无订单!";
	...
	$sql1=mysql_query("select * from tb_dingdan where salerid='".$salerid."' order by time desc limit ".($page-1)*$pagesize.",$pagesize",$conn);
