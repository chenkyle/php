...
if(trim($_FILES['upfile']['name']!=""))
 { 
  $uploadfile="saler/".substr($uploadfile,3);
}
else
 {
  $uploadfile="";
...
session_start();
$salename = $_SESSION["salername"];
$sql=mysql_query( "select * from tb_user where name='".$salename."'and role='卖家' and id='".$salerid."'",$conn);
...
$info=mysql_fetch_array($sql);
      if($info==true)
     {
             ...
             (bool)$result = mysql_query("insert into tb_shangpin(salerID,mingcheng,jianjie,addtime,dengji,tupian,typeid,shichangjia,huiyuanjia,tuijian,shuliang,cishu)values('$salerid','$mingcheng','$jianjie','$addtime','$dengji','$uploadfile','$typeid','$shichangjia','$huiyuanjia','$tuijian','$shuliang','0')",$conn);
		     if($result==true)
				{
